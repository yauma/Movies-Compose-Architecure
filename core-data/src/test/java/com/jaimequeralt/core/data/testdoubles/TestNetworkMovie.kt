...
class TestNetworkMovie : NetworkDataSource {
    private val moshi: Moshi = Moshi.Builder().addLast(KotlinJsonAdapterFactory()).build()
    private val jsonAdapter: JsonAdapter<PopularMoviesResponse> = moshi.adapter((PopularMoviesResponse::class.java))

    private val popularMoviesResponse: PopularMoviesResponse? =
        jsonAdapter.fromJson(popularMoviesResponseJson)

    override suspend fun getPopularMovies(): List<NetworkMovie> {
        return popularMoviesResponse?.results ?: run {
            emptyList()
        }
    }
}

val popularMoviesResponseJson: String = """
        {
   "page":1,
   "results":[
      {
         ...
         "popularity":8079.909,
         "poster_path":"/rUwhgbwn4afF7PFBoQE2BBysFIg.jpg",
